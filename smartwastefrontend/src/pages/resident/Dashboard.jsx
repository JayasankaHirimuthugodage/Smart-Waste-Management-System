...
const ResidentDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const requests = await pickupRequestService.getPickupRequestsByUserId(
          user.userId
        );
  ...
  const renderContent = () => {
    switch (activeNav) {
      ...
      case "requests":
        return <PickupRequestList userId={user?.userId} isAdmin={false} />;
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <div className="mt-6 space-y-2">
                ...
                {pickupRequests.filter(
                  (req) => req.paymentStatus === "COMPLETED"
                ).length === 0 && (
